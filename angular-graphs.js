(function() {
	angular.module("RKGraph", [])
		...
		/**
		 *	linegraph directive definition
		 */
		.directive("linegraph", ['GraphProvider', function(GraphProvider) {
			return {
				restrict: 'E',
				replace: true,
				template: '<svg class="linegraph"></svg>',
				scope: { data: "=inputs" },
				link: function($scope, ele, attr) {
					$scope.$watch('data.length', function() {
						...
						config.svg = ele[0];
						config.showAverage = config.average ? true: false;
						config.averageLineColor = config.average;

						GraphProvider.LineGraph(config);
					});
				}
			};
		}])
		...
		/**
		 *	pointgraph directive definition
		 */
		.directive("pointgraph", ['GraphProvider', function(GraphProvider) {
			return {
				restrict: 'E',
				replace: true,
				template: '<svg class="pointgraph"></svg>',
				scope: { data: "=inputs" },
				link: function($scope, ele, attr) {
					$scope.$watch('data.length', function() {
						var config = attr;
						...
						GraphProvider.PointGraph(config);
					});
				}
			};
		}])
		...
		/**
		 *	bargraph directive definition
		 */
		.directive("bargraph", ['GraphProvider', function(GraphProvider) {
			return {
				restrict: 'E',
				replace: true,
				template: '<svg class="bargraph"></svg>',
				scope: { data: "=inputs" },
				link: function($scope, ele, attr) {
					$scope.$watch('data.length', function() {
						var config = attr;
		...
		/**
		 *	polylinegraph directive definition
		 */
		.directive("polylinegraph", ['GraphProvider', function(GraphProvider) {
			return {
				restrict: 'E',
				replace: true,
				template: '<svg class="polylinegraph"></svg>',
				scope: { data: "=inputs" },
				link: function($scope, ele, attr) {
					$scope.$watch('data.length', function() {
						var config = attr;
						...
						GraphProvider.PolylineGraph(config);
					});
				}
			};
		}])
		...
		/**
		 *	polygongraph directive definition
		 */
		.directive("polygongraph", ['GraphProvider', function(GraphProvider) {
			return {
				restrict: 'E',
				replace: true,
				template: '<svg class="polygongraph"></svg>',
				scope: { data: "=inputs" },
				link: function($scope, ele, attr) {
					$scope.$watch('data.length', function() {
						var config = attr;
						...
						GraphProvider.PolygonGraph(config);
					});
				}
			};
		}])
		...
		/**
		 *	piegraph directive definition
		 */
		.directive("piegraph", ['GraphProvider', function(GraphProvider) {
			return {
				restrict: 'E',
				replace: true,
				template: '<svg class="piegraph"></svg>',
				scope: { data: "=inputs", colors: "=colors", columns: "=columns" },
				link: function($scope, ele, attr) {
					$scope.$watch('data.length', function() {
						var config = attr;
						config.data = $scope.data;
						config.svg = ele[0];
						config.color = $scope.colors;
						config.showAverage = config.average ? true: false;
						config.averageLineColor = config.average;
						config.fixedcolumns = $scope.columns ? true: false;
						config.columns = $scope.columns instanceof Array ? $scope.columns : [];

						GraphProvider.PieGraph(config);
					});
				}
			};
		}]);
